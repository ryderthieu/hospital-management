...
export const scheduleService = {
  ...
  async getSchedulesByDoctorId(doctorId: number): Promise<ScheduleResponse[]> {
    try {
      const response = await api.get<ScheduleResponse[]>(`/doctors/${doctorId}/schedules`);
  ...
  async createSchedule(scheduleData: CreateScheduleRequest): Promise<ScheduleResponse> {
    try {
      const response = await api.post<ScheduleResponse>(`/doctors/${scheduleData.doctorId}/schedules`, scheduleData);
  ...
  async updateSchedule(doctorId: number, scheduleId: number, scheduleData: Partial<CreateScheduleRequest>): Promise<ScheduleResponse> {
    try {
      const response = await api.put<ScheduleResponse>(`/doctors/${doctorId}/schedules/${scheduleId}`, scheduleData);
