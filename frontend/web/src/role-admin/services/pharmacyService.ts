import type { Medicine, MedicineRequest, MedicineUpdateRequest } from "../types/pharmacy"
...
export const medicineService = {
  ...
  async searchMedicine(name?: string, category?: string): Promise<Medicine[]> {
    try {
      ...
      const response = await api.get<Medicine[]>(`/pharmacy/medicines/search?${params.toString()}`)
      return response.data
    } catch (error) {
  ...
  async addMedicine(medicineData: MedicineRequest): Promise<Medicine> {
    try {
      const response = await api.post<Medicine>("/pharmacy/medicines", medicineData)
      return response.data
    } catch (error) {
  ...
  async deleteMedicine(medicineId: number): Promise<string> {
    try {
      const response = await api.delete<string>(`/pharmacy/medicines/${medicineId}`)
      return response.data
    } catch (error) {
  ...
  async getMedicinesByCategory(category: string): Promise<Medicine[]> {
    try {
      const response = await api.get<Medicine[]>(`/pharmacy/medicines/search?category=${category}`)
      return response.data
    } catch (error) {
  ...
  async updateMedicineQuantity(medicineId: number, quantity: number): Promise<Medicine> {
    try {
      const response = await api.put<Medicine>(`/pharmacy/medicines/${medicineId}`, { quantity })
      return response.data
    } catch (error) {
