...
export default function Forgot5({ navigation, route }: Forgot5Props) {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      showAlert({ title: 'Thành công', message: 'Xác thực OTP thành công', buttons: [{ text: 'OK', onPress: () => navigation.navigate('Forgot6', { phone, resetToken }) }] });
    } catch (error: any) {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      showAlert({ title: 'Thành công', message: 'Mã OTP đã được gửi lại' });
    } catch (error: any) {
