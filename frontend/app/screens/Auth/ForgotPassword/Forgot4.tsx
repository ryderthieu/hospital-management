...
type NavigationProp = NativeStackNavigationProp<RootStackParamList, 'Forgot4'>;
type RoutePropType = RouteProp<RootStackParamList, 'Forgot4'>;

interface Forgot4Props {
...
export default function Forgot4({ navigation, route }: Forgot4Props) {
  ...
  const handleVerify = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await API.post<AuthDTOs['OtpValidationResponse']>('/users/auth/otp/validate-email', null, {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.container, styles.scrollContainer}>
        <PageHeader
          ...
          subtitle={
            <Text>
              Vui lòng nhập OTP 6 số mà chúng tôi đã gửi đến email{' '}
