  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      const filteredData = Array.isArray(response.data.content)
        ? response.data.content.filter((dto) => statusFilter.includes(dto.appointmentStatus))
        : [];

      const appointments: Appointment[] = await Promise.all(
        filteredData.map(async (dto) => {
          let doctorInfo = dto.doctorInfo;
          if (!doctorInfo || !doctorInfo.avatar) {
            try {
              const doctorResponse = await API.get(`/doctors/${dto.doctorId}`);
              console.log(`[AppointmentsScreen] Doctor ${dto.doctorId} response:`, doctorResponse.data);
          ...
          return {
            ...
            image: doctorInfo?.avatar ? { uri: doctorInfo.avatar.trim() } : null, // Loại bỏ \n
            status: activeTab,
